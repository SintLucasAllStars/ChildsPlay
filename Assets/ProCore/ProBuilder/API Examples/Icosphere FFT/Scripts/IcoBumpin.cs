...
namespace ProBuilder2.Examples
{
    ...
    {
        private const float TWOPI = 6.283185f; // 2 * PI
        private const int WAVEFORM_SAMPLES = 1024; // How many samples make up the waveform ring.
        private const int FFT_SAMPLES = 4096; // How many samples are used in the FFT.  More means higher resolution.
        ...
        [Range(1f, 50f)] public float extrusion = 30f;
        ...
        private readonly float[] fft = new float[FFT_SAMPLES];

        private readonly float[] fft_history = new float[FFT_SAMPLES];

        private readonly float[] data = new float[WAVEFORM_SAMPLES];

        private readonly float[] data_history = new float[WAVEFORM_SAMPLES];
        ...
        [Range(8, 128)] public int fftBounds = 32;

        // Optionally weights the frequency amplitude when calculating extrude distance.
        public AnimationCurve frequencyCurve;
        private pb_Object ico; // A reference to the icosphere pb_Object component

        private Mesh
            icoMesh; // A reference to the icosphere mesh (cached because we access the vertex array every frame)

        // Icosphere's starting position.
        private Vector3 icoPosition = Vector3.zero;

        // The radius of the mesh icosphere on instantiation.
        [Range(1f, 10f)] public float icoRadius = 2f;

        // The number of subdivisions to give the icosphere.
        [Range(0, 3)] public int icoSubdivisions = 2;

        private Transform
            icoTransform; // A reference to the icosphere transform component.  Cached because I can't remember if GameObject.transform is still a performance drain :|
        ...
        [Range(0f, 1f)] public float startingExtrusion = .1f;

        // How high the icosphere transform will bounce (sample volume determines height).
        [Range(0f, 10f)] public float verticalBounce = 4f;
        ...
        private void Update()
        {
            ...
            {
                var normalizedIndex = i / faces_length;

                var n = (int) (normalizedIndex * fftBounds);

                var displacement = outsides[i].nrm *
                                   ((fft[n] + fft_history[n]) * .5f *
                                    (frequencyCurve.Evaluate(normalizedIndex) * .5f + .5f)) * extrusion;

                foreach (var t in outsides[i].indices) displaced_vertices[t] = original_vertices[t] + displacement;
            }
            ...
            {
                var n = i < WAVEFORM_SAMPLES - 1 ? i : 0;
                vec.x = Mathf.Cos((float) n / WAVEFORM_SAMPLES * TWOPI) *
                        (waveformRadius + (data[n] + data_history[n]) * .5f * waveformHeight);
                vec.z = Mathf.Sin((float) n / WAVEFORM_SAMPLES * TWOPI) *
                        (waveformRadius + (data[n] + data_history[n]) * .5f * waveformHeight);
            ...
            Array.Copy(fft, fft_history, FFT_SAMPLES);
            Array.Copy(data, data_history, WAVEFORM_SAMPLES);
            rms_history = rms;
        ...
        {
            public pb_Face face;
            public readonly Vector3 nrm; // face normal
            public readonly int[] indices; // all vertex indices (including shared connected vertices)
