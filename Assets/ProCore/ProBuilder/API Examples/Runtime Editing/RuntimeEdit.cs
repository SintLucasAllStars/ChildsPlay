...
namespace ProBuilder2.Examples
{
    ...
    public class RuntimeEdit : MonoBehaviour
    {
        ...
        public void LateUpdate()
        {
            ...
            if (dragging)
            {
                Vector2 delta = (Vector3) mousePosition_initial - Input.mousePosition;
            ...
            if (Input.GetMouseButtonUp(1) || Input.GetMouseButtonUp(0)) dragging = false;
        }
            ...
            if (Input.GetMouseButtonUp(0) && !Input.GetKey(KeyCode.LeftAlt))
                if (FaceCheck(Input.mousePosition))
                    if (currentSelection.IsValid())
                    {
                        ...
                        var localNormal =
                            pb_Math.Normal(
                                currentSelection.pb.vertices.ValuesWithIndices(currentSelection.face.distinctIndices));

                        if (Input.GetKey(KeyCode.LeftShift))
                            currentSelection.pb.TranslateVertices(currentSelection.face.distinctIndices,
                                localNormal.normalized * -.5f);
                        else
                            currentSelection.pb.TranslateVertices(currentSelection.face.distinctIndices,
                                localNormal.normalized * .5f);
        ...
        private void RefreshSelectedFacePreview()
        {
            ...
            preview = pb_Object.CreateInstanceWithVerticesFaces(verts, new[] {new pb_Face(indices)});
