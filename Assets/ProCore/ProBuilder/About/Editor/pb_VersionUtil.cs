...
namespace ProBuilder2.EditorCommon
{
    ...
    internal static class pb_VersionUtil
    {
        ...
        public static bool GetAboutEntry(out pb_AboutEntry about)
        {
            ...
            for (var i = 0; i < matches.Length && about == null; i++)
        ...
        public static bool FormatChangelog(string raw, out pb_VersionInfo version, out string formatted_changes)
        {
            ...
            try
            {
                ...
                version = pb_VersionInfo.FromString(versionMatch.Success
                    ? versionMatch.Value
                    : split[1].Split('\n')[0]);
            }
            catch
            ...
            try
            {
                ...
                formatted_changes = Regex.Replace(formatted_changes, @"(?<=^##\\s).*", "<size=16><b>${0}</b></size>",
