...
namespace ProBuilder2.EditorCommon
{
    ...
    public class pb_MissingScriptEditor : Editor
    {
         ...
         */
        private readonly List<string> PB_ENTITY_SCRIPT_PROPERTIES = new List<string>
        {
            "pb",
            "userSetDimensions",
            "_entityType",
         ...
         */
        private readonly List<string> PB_OBJECT_SCRIPT_PROPERTIES = new List<string>
        {
            "_sharedIndices",
            "_vertices",
            "_uv",
        ...
        private static void FixAllScriptReferencesInScene()
        {
            ...
            var all = Resources.FindObjectsOfTypeAll(typeof(GameObject))
                .Where(x => ((GameObject) x).GetComponents<Component>().Any(n => n == null)).ToArray();
            total = all.Length;
        ...
        private static void Next()
        {
            ...
            if (EditorUtility.DisplayCancelableProgressBar("Repair ProBuilder Script References",
                "Fixing " + (int) Mathf.Floor(index + 1) + " out of " + total + " objects in scene.", index / total))
            {
                earlyExit = true;
                doFix = false;
            }
            ...
            if (!earlyExit)
                foreach (GameObject go in Resources.FindObjectsOfTypeAll(typeof(GameObject)))
                    if (go.GetComponents<Component>().Any(x => x == null) && !unfixable.Contains(go))
                    {
                        ...
                        {
                            var pref = (GameObject) PrefabUtility.GetPrefabParent(go);
            ...
            {
                EditorUtility.DisplayProgressBar("Checking ProBuilder Meshes",
                    "Refresh " + (i + 1) + " out of " + total + " objects in scene.", (float) i / pbs.Length);
                ...
                {
                    Debug.LogWarning("Failed reconstituting " + pbs[i].name +
                                     ".  Proceeding with upgrade anyways.  Usually this means a prefab is already fixed, and just needs to be instantiated to take effect.\n" +
                                     e);
                }
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (scriptProperty == null || scriptProperty.objectReferenceValue != null)
            {
                if (doFix)
                {
                    if (Event.current.type == EventType.Repaint) Next();
                }
                else
                {
                    base.OnInspectorGUI();
            ...
            if (pbObjectMatches >= 3 || pbEntityMatches >= 3)
            {
                EditorGUILayout.HelpBox(
                    "Missing Script Reference\n\nProBuilder can automatically fix this missing reference.  To fix all references in the scene, click \"Fix All in Scene\".  To fix just this one, click \"Reconnect\".",
                    MessageType.Warning);
            }
            else
            {
            ...
            if (doFix && Event.current.type == EventType.Repaint || GUILayout.Button("Reconnect"))
            {
                ...
                ) // only increment for pb_Object otherwise the progress bar will fill 2x faster than it should
                {
                    index++;
                }
                else
                ...
                if (!doFix) Undo.RegisterCompleteObjectUndo(target, "Fix missing reference.");
        ...
        private static void DeleteDummyScripts()
        {
            ...
            if (dummies.Length > 0)
            {
                var ret = EditorUtility.DisplayDialogComplex("Found Unrepairable Objects",
                    "Repair script found " + dummies.Length +
                    " missing components that could not be repaired.  Would you like to delete those components now, or attempt to rebuild (ProBuilderize) them?",
                    "Delete", "Cancel", "ProBuilderize");
                ...
                switch (ret)
                {
                    ...
                    default:
                    {
                        ...
                        if (ret == 2)
                        {
                            ...
                            var broken = Resources.FindObjectsOfTypeAll(typeof(GameObject))
                            ...
                            ProBuilderize.DoProBuilderize(
                                Array.ConvertAll(broken, x => (GameObject) x)
        ...
        private static string SerializedObjectToString(SerializedObject serializedObject)
        {
            ...
            {
                var tabs = "";
                for (var i = 0; i < iterator.depth; i++) tabs += "\t";

                sb.AppendLine(tabs + iterator.name +
                              (iterator.propertyType == SerializedPropertyType.ObjectReference &&
                               iterator.type.Contains("Component") && iterator.objectReferenceValue == null
                                  ? " -> NULL"
                                  : ""));

                tabs += "  - ";

                sb.AppendLine(tabs + "Type: (" + iterator.type + " / " + iterator.propertyType + " / " + " / " +
                              iterator.name + ")");
        ...
        private static readonly bool applyDummyScript = true;

	    /// < If true, any null components that
	    ///     can't be set will have this script applied to their reference, allowing us to later remove them.
	    private static float index;

        ///< general idea of where we are in terms of processing this scene.
        private static float total;

        ///< general idea of how many missing script references are in this scene.
        private static bool doFix;

        ///< while true, the inspector will attempt to cycle to broken gameobjects until none are found.
        private static readonly List<GameObject> unfixable = new List<GameObject>();

	    /// < if a non-pb missing reference is encountered, need to let the iterator know not to bother
	    /// ,
