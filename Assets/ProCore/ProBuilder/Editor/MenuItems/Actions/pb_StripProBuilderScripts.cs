...
namespace ProBuilder2.Actions
{
    public class pb_StripProBuilderScripts : Editor
    {
        ...
        {
            if (!EditorUtility.DisplayDialog("Strip ProBuilder Scripts",
                "This will remove all ProBuilder scripts in the scene.  You will no longer be able to edit these objects.  There is no undo, please exercise caution!\n\nAre you sure you want to do this?",
                "Okay", "Cancel"))
                return;
        ...
        {
            if (!EditorUtility.DisplayDialog("Strip ProBuilder Scripts",
                "This will remove all ProBuilder scripts on the selected objects.  You will no longer be able to edit these objects.  There is no undo, please exercise caution!\n\nAre you sure you want to do this?",
                "Okay", "Cancel"))
                return;

            foreach (var t in Selection.transforms)
            foreach (var pb in t.GetComponentsInChildren<pb_Object>(true))
                DoStrip(pb);
        }
        ...
        public static void Strip(pb_Object[] all)
        {
            ...
            EditorUtility.DisplayDialog("Strip ProBuilder Scripts",
                "Successfully stripped out all ProBuilder components.", "Okay");
        ...
        public static void DoStrip(pb_Object pb)
        {
            try
            {
                ...
                if (pb_PreferencesInternal.GetBool(pb_Constant.pbMeshesAreAssets) &&
                    pb_EditorMeshUtility.GetCachedMesh(pb, out cachedMeshPath, out cachedMesh))
                {
