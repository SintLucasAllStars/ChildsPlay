...
namespace ProBuilder2.Actions
{
    ...
    {
        [MenuItem("Tools/" + pb_Constant.PRODUCT_NAME + "/Actions/Generate UV2 - Selection", true,
            pb_Constant.MENU_ACTIONS + 20)]
        public static bool VerifyGenerateUV2Selection()
        {
            return Selection.transforms.GetComponents<pb_Object>().Length > 0;
        }

        [MenuItem("Tools/" + pb_Constant.PRODUCT_NAME + "/Actions/Generate UV2 - Selection", false,
            pb_Constant.MENU_ACTIONS + 20)]
        ...
        [MenuItem("Tools/" + pb_Constant.PRODUCT_NAME + "/Actions/Generate UV2 - Scene", false,
            pb_Constant.MENU_ACTIONS + 20)]
            ...
            for (var i = 0; i < selected.Length; i++)
            {
                if (selected.Length > 3)
                        ...
                        ((float) i + 1) / selected.Length))
                    {
                        ...
                        Debug.LogWarning("User canceled UV2 generation.  " + (selected.Length - i) +
                                         " pb_Objects left without lightmap UVs.");
                        return false;
                    }
