...
public class pb_Preferences : Editor
{
    ...
    private static Vector2 settingsScroll = Vector2.zero;
    private static bool pbShowEditorNotifications;
    private static bool pbForceConvex;
    private static bool pbDragCheckLimit;
    ...
    private static bool pbCloseShapeWindow;
    private static bool pbUVEditorFloating = true;
    private static bool pbStripProBuilderOnBuild = true;
    private static bool pbDisableAutoUV2Generation;
    private static bool pbShowSceneInfo;
    private static bool pbUniqueModeShortcuts;
    private static bool pbIconGUI;
    private static bool pbShiftOnlyTooltips;
    private static bool pbDrawAxisLines = true;
    private static bool pbMeshesAreAssets;
    private static bool pbElementSelectIsHamFisted;
    private static bool pbDragSelectWholeElement;
    private static bool pbEnableExperimental;

    private static bool showMissingLightmapUvWarning;

#if !UNITY_4_6 && !UNITY_4_7
    private static ShadowCastingMode pbShadowCastingMode = ShadowCastingMode.On;
#endif

    private static ColliderType defaultColliderType = ColliderType.BoxCollider;
    private static SceneToolbarLocation pbToolbarLocation = SceneToolbarLocation.UpperCenter;
    private static EntityType pbDefaultEntity = EntityType.Detail;
    ...
    private static void PreferencesGUI()
    {
        ...
        settingsScroll =
            EditorGUILayout.BeginScrollView(settingsScroll, GUILayout.MinHeight(180), GUILayout.MaxHeight(180));
        ...
        pbShowSceneInfo =
            EditorGUILayout.Toggle(
                new GUIContent("Show Scene Info",
                    "Displays the selected object vertex and triangle counts in the scene view."), pbShowSceneInfo);
        ...
        pbShiftOnlyTooltips =
            EditorGUILayout.Toggle(
                new GUIContent("Shift Key Tooltips", "Tooltips will only show when the Shift key is held"),
                pbShiftOnlyTooltips);
        ...
        pbDefaultEntity = (EntityType) EditorGUILayout.EnumPopup(pbDefaultEntity);
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        EditorGUILayout.PrefixLabel("Default Collider");
        defaultColliderType = (ColliderType) EditorGUILayout.EnumPopup(defaultColliderType);
        GUILayout.EndHorizontal();

        if (defaultColliderType == ColliderType.MeshCollider)
        ...
        pbDefaultSelectedVertexColor =
            EditorGUILayout.ColorField("Selected Vertex Color", pbDefaultSelectedVertexColor);
...
#pragma warning restore 0618
    ...
    private static int shortcutIndex;

#if UNITY_5_6_OR_HIGHER
    private static readonly Rect selectBox = new Rect(0, 214, 183, 156);
    private static readonly Rect shortcutEditRect = new Rect(190, 191, 178, 300);
#else
	static Rect selectBox = new Rect(130, 253, 183, 142);
	static Rect shortcutEditRect = new Rect(320, 228, 178, 300);
#endif

    private static Vector2 shortcutScroll = Vector2.zero;
    private static readonly int CELL_HEIGHT = 20;
    ...
    private static void ShortcutSelectPanel()
    {
        ...
        shortcutScroll = EditorGUILayout.BeginScrollView(shortcutScroll, false, true, GUILayout.MaxWidth(183),
            GUILayout.MaxHeight(156));

        for (var n = 1; n < defaultShortcuts.Length; n++)
            if (n == shortcutIndex)
            {
            ...
            else
            {
                if (GUILayout.Button(defaultShortcuts[n].action, labelStyle, GUILayout.MinHeight(CELL_HEIGHT),
                    GUILayout.MaxHeight(CELL_HEIGHT))) shortcutIndex = n;
            }
    ...
    private static void ShortcutEditPanel()
    {
        ...
        var em = defaultShortcuts[shortcutIndex].eventModifiers;
    ...
    public static void SetPrefs()
    {
        ...
        pb_PreferencesInternal.SetInt(pb_Constant.pbToolbarLocation, (int) pbToolbarLocation,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetInt(pb_Constant.pbDefaultEntity, (int) pbDefaultEntity);

        pb_PreferencesInternal.SetColor(pb_Constant.pbDefaultFaceColor, pbDefaultFaceColor,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetColor(pb_Constant.pbDefaultEdgeColor, pbDefaultEdgeColor,
            pb_PreferenceLocation.Global);
        ...
        pb_PreferencesInternal.SetString(pb_Constant.pbDefaultShortcuts,
            pb_Shortcut.ShortcutsToString(defaultShortcuts), pb_PreferenceLocation.Global);
        ...
        pb_PreferencesInternal.SetBool(pb_Constant.pbForceVertexPivot, pbForceVertexPivot,
            pb_PreferenceLocation.Global);
        ...
        pb_PreferencesInternal.SetBool(pb_Constant.pbPerimeterEdgeBridgeOnly, pbPerimeterEdgeBridgeOnly,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetBool(pb_Constant.pbPBOSelectionOnly, pbPBOSelectionOnly,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetBool(pb_Constant.pbCloseShapeWindow, pbCloseShapeWindow,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetBool(pb_Constant.pbUVEditorFloating, pbUVEditorFloating,
            pb_PreferenceLocation.Global);
        ...
        pb_PreferencesInternal.SetBool(pb_Constant.pbShiftOnlyTooltips, pbShiftOnlyTooltips,
            pb_PreferenceLocation.Global);
        ...
        pb_PreferencesInternal.SetBool(pb_Constant.pbDragSelectWholeElement, pbDragSelectWholeElement,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetBool(pb_Constant.pbEnableExperimental, pbEnableExperimental,
            pb_PreferenceLocation.Global);
        pb_PreferencesInternal.SetBool("pb_Lightmapping::showMissingLightmapUvWarning", showMissingLightmapUvWarning,
            pb_PreferenceLocation.Global);

        pb_PreferencesInternal.SetFloat(pb_Constant.pbVertexHandleSize, pbVertexHandleSize,
            pb_PreferenceLocation.Global);
